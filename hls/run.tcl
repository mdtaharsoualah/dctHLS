...
for {set i 1} {$i < 17} {incr i} {
	...
	foreach l {2 4 8 16} {
		...
		incr m
		open_solution -reset "solUnroll$i$tmp$l"
		set_part {xc7z020clg484-1} -tool vivado
		create_clock -period 10 -name default
		
		foreach k $myArray($i) {
			if {$k != 0} {
				set_directive_unroll -factor $l "dct/Loop$k"
			}
		}
...
for {set i 1} {$i < 17} {incr i} {
	...
	for {set l 20} {$l > 2} {incr l -2} {
		
		open_solution -reset "solLatence$i$tmp$l"
		set_part {xc7z020clg484-1} -tool vivado
		create_clock -period 10 -name default
		
		foreach k $myArray($i) {
			if {$k != 0} {
				set_directive_latency -max $l "dct/Loop$k"
			}
		}
...
for {set i 1} {$i < 17} {incr i} {
	...
	open_solution -reset "solFlatten$i"
	set_part {xc7z020clg484-1} -tool vivado
	create_clock -period 10 -name default
	
	foreach k $myArray($i) {
		if {$k != 0} {
			set_directive_loop_flatten "dct/Loop$k"
		}
	}
...
for {set i 1} {$i < 17} {incr i} {
	...
	incr m
	open_solution -reset "solMerge$i"
	set_part {xc7z020clg484-1} -tool vivado
	create_clock -period 10 -name default
	
	foreach k $myArray($i) {
		if {$k != 0} {
			set_directive_loop_merge "dct/Loop$k"
		}
	}
